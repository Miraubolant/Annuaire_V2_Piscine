...
$faq = [
    ['q' => "Combien y a-t-il de isolants à {$villeNom} ?",
     'r' => "{$villeNom} compte " . getCompteurLabel($nbArtisans) . " référencés dans notre annuaire. Pour obtenir un devis adapté à votre projet d'isolation, comparez plusieurs artisans et vérifiez leur certification RGE."],
    ['q' => "Quelles aides sont disponibles pour des travaux d'isolation {$artVille} ?",
     'r' => "Les habitants de {$villeNom} bénéficient de la TVA à 5,5% pour tous les travaux d'isolation thermique, de la prime CEE (BAR-EN-101 combles, BAR-EN-102 murs, BAR-EN-103 planchers), de l'Éco-PTZ et de MaPrimeRénov'. La zone climatique {$zone} détermine le montant des primes CEE."],
    ['q' => "La TVA à 5,5% s'applique-t-elle pour des travaux d'isolation {$artVille} ?",
     'r' => "Oui, la TVA à 5,5% s'applique à tous les travaux d'isolation thermique (combles, murs, planchers) dans les logements de plus de 2 ans à {$villeNom}. C'est le taux le plus bas applicable, représentant une économie de 14,5% par rapport au taux normal."],
    ['q' => "Quelle est la meilleure période pour faire des travaux d'isolation {$artVille} ?",
     'r' => "Les travaux d'isolation intérieure (combles, planchers) se réalisent toute l'année. L'ITE (isolation par l'extérieur) est idéalement réalisée au printemps ou en été. Anticipez en contactant votre isolant {$artVille} dès l'automne précédent."],
    ['q' => "Comment choisir un isolant {$artVille} ?",
     'r' => "Pour choisir un isolant à {$villeNom}, vérifiez sa certification RGE (indispensable pour les aides), ses qualifications (QualiPAC, RGE Eco Artisan), lisez les avis clients et comparez au moins 3 devis détaillés."],
    ['q' => "Quel est le coût de l'isolation des combles à {$villeNom} ?",
     'r' => "L'isolation des combles perdus par soufflage coûte entre 20 € et 50 €/m² à {$villeNom}. Sur 100 m², comptez entre 2 000 € et 5 000 €. Après aides CEE (zone {$zone}) et MaPrimeRénov', le reste à charge peut descendre à moins de 500 €."],
    ['q' => "{$pct}% des logements de {$villeNom} ont été construits avant 1990 — est-ce important ?",
     'r' => "Oui, les logements construits avant 1990 ont souvent des toitures vieillissantes et des combles mal isolés. Ils sont prioritaires pour les aides à la rénovation : TVA à 5,5% sur l'isolation, prime CEE BAR-EN-101, Éco-PTZ et MaPrimeRénov'."],
    ['q' => "Peut-on obtenir plusieurs devis de isolants {$artVille} gratuitement ?",
     'r' => "Absolument. Il est recommandé de demander au moins 3 devis comparatifs. Notre annuaire vous permet de contacter directement les isolants de {$villeNom} et de demander des devis gratuits via notre formulaire."],
    ['q' => "Les isolants de {$villeNom} sont-ils certifiés RGE ?",
     'r' => "Oui, tous les isolants référencés dans notre annuaire possèdent ou peuvent justifier d'une certification RGE (Reconnu Garant de l'Environnement). Cette certification est indispensable pour bénéficier des aides CEE, MaPrimeRénov' et l'Éco-PTZ."],
    ['q' => "Comment bénéficier de la prime CEE pour l'isolation des combles {$artVille} ?",
     'r' => "Pour obtenir la prime CEE à {$villeNom}, l'isolation de vos combles perdus doit respecter la résistance thermique R≥7 m².K/W, être réalisée par un isolant certifié RGE, et l'accord signé avant le début des travaux."],
];
    ...
    <div class="ph-grid">
        ...
        <div>
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:20px;">
                ...
                <span style="color:rgba(255,255,255,.4);font-size:13px;">· 65 000 isolants en France</span>
            </div>
            ...
            <h1 style="font-family:var(--font-display);font-size:clamp(26px,3.5vw,44px);font-weight:700;color:#fff;line-height:1.2;margin-bottom:14px;letter-spacing:-.02em;">
                isolant <?= htmlspecialchars($artVille) ?><br>
            ...
            <p style="font-size:15px;color:rgba(255,255,255,.65);margin-bottom:28px;line-height:1.7;">
                Trouvez un isolant certifié RGE à <strong style="color:#fff;"><?= htmlspecialchars($villeNom) ?></strong> — devis gratuit, TVA 5,5%, prime CEE disponibles.
            </p>
            ...
            <div>
                <p style="font-size:11px;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.08em;font-weight:700;margin-bottom:10px;">Nos services isolation <?= htmlspecialchars($artVille) ?></p>
                <div class="ph-tags">
        ...
        <div class="lg:col-span-2 space-y-10">
            ...
            <section id="artisans">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
                    <h2 style="font-family:var(--font-display);font-size:20px;font-weight:700;color:var(--text);">
                        Les isolants recommandés <?= htmlspecialchars($artVille) ?>
                    </h2>
                ...
                <div style="background:var(--stone);border-radius:var(--radius-lg);padding:32px;text-align:center;">
                    <p style="font-weight:600;color:var(--text);margin-bottom:6px;">Aucun isolant référencé à <?= htmlspecialchars($villeNom) ?> pour l'instant.</p>
            ...
            <section>
                ...
                <div style="background:#fff;border-radius:var(--radius-lg);border:1px solid var(--stone);overflow:hidden;box-shadow:var(--shadow-sm);">
                    <iframe src="<?= htmlspecialchars($mapUrl) ?>" width="100%" height="300" style="border:none;display:block;" loading="lazy" referrerpolicy="no-referrer-when-downgrade" title="isolants <?= htmlspecialchars($villeNom) ?>"></iframe>
                    <div style="padding:12px 20px;background:var(--cream);display:flex;align-items:center;justify-content:space-between;font-size:12px;color:var(--text-muted);">
                        <span><?= htmlspecialchars($villeNom) ?> · <?= $nbArtisans ?> isolant<?= $nbArtisans > 1 ? 's' : '' ?> référencé<?= $nbArtisans > 1 ? 's' : '' ?></span>
                        <a href="https://www.google.com/maps/search/isolant+<?= urlencode($villeNom . ' ' . $villeCp) ?>/" target="_blank" rel="noopener" style="color:var(--forest);font-weight:600;text-decoration:none;">Voir sur Google Maps →</a>
                    </div>
                </div>
            </section>
            <?php endif; ?>
            ...
            <section>
                <h2 style="font-family:var(--font-display);font-size:20px;font-weight:700;color:var(--text);margin-bottom:8px;">
                    Services isolation <?= htmlspecialchars($artVille) ?>
                </h2>
                <p style="font-size:14px;color:var(--text-muted);margin-bottom:16px;">
                    Découvrez nos 40 prestations — nos artisans locaux vous accompagnent de l'étude au chantier.
                </p>
            ...
            <section>
                <h2 style="font-family:var(--font-display);font-size:22px;font-weight:700;color:var(--text);margin-bottom:12px;">
                    Votre isolant expert <?= htmlspecialchars($artVille) ?> (<?= htmlspecialchars($villeCp) ?>)
                </h2>
                <?php if ($seoText): ?>
                ...
                <div style="background:#fff;border-radius:var(--radius-lg);border:1px solid var(--stone);padding:24px;margin-bottom:24px;box-shadow:var(--shadow-sm);">
                    <h3 style="font-weight:700;color:var(--text);margin-bottom:16px;">Pourquoi choisir nos isolants <?= htmlspecialchars($artVille) ?> ?</h3>
                    <ul style="display:flex;flex-direction:column;gap:12px;list-style:none;">
                        ...
                        <li style="display:flex;align-items:center;gap:10px;font-size:14px;color:var(--text);">
                            <span style="color:var(--forest);font-weight:700;font-size:16px;">✓</span> TVA à 5,5% sur tous les travaux d'isolation thermique
                        </li>
                        <li style="display:flex;align-items:center;gap:10px;font-size:14px;color:var(--text);">
                ...
                <div class="grid sm:grid-cols-3 gap-4">
                    ...
                    <div style="background:#fff;border-radius:var(--radius-lg);border:1px solid var(--stone);padding:24px;text-align:center;box-shadow:var(--shadow-sm);">
                        ...
                        <p style="font-size:13px;color:var(--text-muted);">Recevez 3 propositions de isolants <?= htmlspecialchars($artVille) ?>.</p>
                    </div>
                    <div style="background:#fff;border-radius:var(--radius-lg);border:1px solid var(--stone);padding:24px;text-align:center;box-shadow:var(--shadow-sm);">
            ...
            <?php
            $questions = $faq;
            $title = 'Questions fréquentes — isolant ' . $artVille;
            ...
            <section>
                <h2 style="font-family:var(--font-display);font-size:20px;font-weight:700;color:var(--text);margin-bottom:16px;">
                    isolants à proximité de <?= htmlspecialchars($villeNom) ?>
                </h2>
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    ...
                    <a href="<?= htmlspecialchars($procheUrl) ?>"
                       ...
                       onmouseout="this.style.borderColor='var(--stone)';this.style.boxShadow='none';">
                        ...
                        <p style="font-weight:600;color:var(--text);font-size:13px;margin-bottom:2px;">isolant <?= htmlspecialchars($proche['nom']) ?></p>
                ...
                <div style="background:#fff;border-radius:var(--radius-lg);border:1px solid var(--stone);padding:24px;box-shadow:var(--shadow-sm);">
                    ...
                    <ul style="display:flex;flex-direction:column;gap:10px;list-style:none;">
                        ...
                        <li style="display:flex;justify-content:space-between;font-size:13px;">
                            <span style="color:var(--text-muted);">isolants référencés</span>
